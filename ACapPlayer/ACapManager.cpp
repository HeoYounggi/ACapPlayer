...
bool CACapManager::Stop()
{
	...
	//::PostMessage(m_hOwner, WM_ACAP_UPDATE_STATUS);
...
void CACapManager::OnUpdateReportReader(TCHAR* m_szFilePath)
{
	...
	if (m_szFilePath != NULL)
	{
		...
		::PostMessage(m_hOwner, WM_SEND_FILE_NAME_MANAGER, wParam, lParam);
	}
...
void CACapManager::OnUpdateReportTotalPacket(unsigned int m_unTotalPacket)
{
	WPARAM wParam = NULL;
	if (m_unTotalPacket != 0)
	{
		...
		TRACE(TEXT("%d"), lParam);
		::PostMessage(m_hOwner, WM_ACAP_UPDATE_STATUS, wParam, lParam);
	}
