...
BEGIN_MESSAGE_MAP(CACapPlayerView, CFormView)
	...
	ON_MESSAGE(WM_ACAP_SEND_FILE_NAME,&CACapPlayerView::OnSendFileName)
	ON_MESSAGE(WN_ACAP_TOTAL_PACKET_COUNT,&CACapPlayerView::TotalPacket)
	ON_WM_DESTROY()
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
void CACapPlayerView::OnBnClickedButtonMainPlay()
{
	...
	int nLoops = m_Loop_Spin.GetPos();
	int nDelay = m_DelayLoop_Spin.GetPos();
	if(m_Loop_CheckBox.GetCheck() == BST_UNCHECKED)
	{
		nLoops = 1;
		nDelay = 1000;
	}
	m_pcManager->GetLoopDelayCount(nLoops,nDelay);

	//m_pcManager->GetTotalPacket();
...
{
	LPCTSTR m_pszFileName = (LPCTSTR)lParam;
	CString m_strSendingFileName = m_pszFileName;

	m_Current_Text.SetWindowTextW(m_strSendingFileName);

	return 0;
}
//==============================================================================

afx_msg LRESULT CACapPlayerView::TotalPacket(WPARAM wParam, LPARAM lParam)
{
	unsigned int m_lstrTotalCount = lParam;

	m_strTotalCounut.Format(TEXT("%d"),m_lstrTotalCount);
